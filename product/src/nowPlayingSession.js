...
function waitForMetadata(element) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const failed = () => { cleanup(); reject(element.error ?? new Error("Unable to load lock-screen audio.")); };
...
async function syncAudio(media, { allowStart = false } = {}) {
    ...
    if (Math.abs(element.currentTime - position) > SEEK_TOLERANCE_SECONDS) {
        try { element.currentTime = position; } catch { /* Safari can reject seeks while loading. */ }
    }
...
function updateMediaSession(media) {
    ...
    navigator.mediaSession.metadata = media?.title
        ? new MediaMetadata({
            title: media.title,
            artist: media.channelName || "PipedKodi",
            album: "PipedKodi",
...
export function createNowPlayingSession({ getMedia, control }) {
    ...
    const activate = () => { activationRequested = true; void maybeActivate(); };
    ...
    if ("mediaSession" in navigator) {
        const handlers = {
            play: () => { if (getMedia()?.paused) void control({ action: "playpause" }); },
            pause: () => { if (!getMedia()?.paused) void control({ action: "playpause" }); },
