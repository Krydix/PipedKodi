...
export const api = {
    ...
    watchContextContinuation: (id, continuation, replies = false) => request(`/api/catalog/watch/${encodeURIComponent(id)}/continuation`, { method: "POST", body: JSON.stringify({ continuation, replies }) }),
    ...
    kodiPlay: (type, id, resume = false) => request("/api/kodi/play", { method: "POST", body: JSON.stringify({ type, id, resume }) }),
    kodiWatched: (type, id, watched) => request("/api/kodi/watched", { method: "POST", body: JSON.stringify({ type, id, watched }) }),
    kodiInput: (action, value) => request("/api/kodi/input", { method: "POST", body: JSON.stringify({ action, value }) }),
